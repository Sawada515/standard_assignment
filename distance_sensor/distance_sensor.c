...
#define calc_distance(ch) \
(double)((26.549 * pow(read_adc((ch)) * 0.002, -1.2091F)))

static void set_read_channel(enum AdcChannel ch);
static short read_adc(enum AdcChannel ch);
static unsigned char adc_is_busy(void);
...
void *get_distance_sensor_thread(void *arg)
{
	...
	while(1) {
		for(signed char i = 0; i < 4; ++i) {
			set_read_channel(ch[i]);

			ret = calc_distance(ch[i]);
...
#ifndef DEBUG
	i2c_write_word_data(PigpioDaemon, I2C_DevHandle, CONFIG_REG, config);
#endif
}
...
#ifndef DEBUG
	ret = i2c_read_word_data(PigpioDaemon, I2C_DevHandle, CONVERT_REG);
#else
	ret = 0x00FF;
#endif
...
{
#ifndef DEBUG
	short ret = i2c_read_word_data(PigpioDaemon, I2C_DevHandle, CONFIG_REG);
#else
	short ret = 0x0000;
#endif
