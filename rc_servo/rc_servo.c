...
#define SERVO 		14
#define PERIOD		15000		//15[ms]	

#ifndef DEBUG
#define SET_HIGH()  	gpio_write(this->pigpio_daemon, SERVO, 1)
#define SET_LOW()	gpio_write(this->pigpio_daemon, SERVO, 0)
#else
#define SET_HIGH() 	printf("HIGH\n")
#define SET_LOW()	printf("LOW\n")	
#endif
...
void *rc_servo_control_thread(void *arg)
{
	...
	while(1) {
		...
		wait_time = map(rc_servo->angle, 0, 180, 8, 23) * 100;

		usleep(wait_time);

		SET_LOW();

		usleep(PERIOD - wait_time);
	}
